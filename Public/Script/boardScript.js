...
function GameBoardInitialization () {
  const gameBoard = document.getElementById('game-board') // gameBoard is of type HTMLElement

  for (let i = 0; i < 6; i++) { // Create 6 word containers
    const wordContainer = document.createElement('div') // wordContainer is of type HTMLElement
    wordContainer.className = 'letter-row'

    for (let j = 0; j < 5; j++) { // Create 5 letter container for each wordContainer
      const letterContainer = document.createElement('div') // letterContainer is of type HTMLElement
...
function ResetGameBoard () {
  ...
  const word = Array.from(gameBoard.children)
  // console.log(word)
  // console.log(word.length)
  for (let i = 0; i < word.length; i++) {
    const letters = Array.from(word[i].children)
    for (let j = 0; j < 5; j++) {
...
function UpdateWordContainer (guessesWord, guessNo) {
  const wordContainer = document.getElementsByClassName('letter-row')[guessNo] // wordContainer is specified by guessNo
...
function ChangeLetterContainerColour (boxCorrectnessStatus, guessNo, letterNo) {
  const wordContainer = document.getElementsByClassName('letter-row')[guessNo] // wordContainer is specified by guessNo
  const letterContainer = wordContainer.children[letterNo] // letterContainer is specified by letterNo
